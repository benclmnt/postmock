...
| `src/recipients/test-bounces.ts`, `src/plugins/test-bounces.ts` | Fake bounces of `bounce-testing.postmarkapp.com`: `validateOutbound` refuses uncaptured types before storing; the plugin schedules the bounce on the clock after `sent` (`docs/07` §2.3) | built |
