...
| `POST /control/latency` | `{match: {method, path, recipientDomain?}, times?, ms}` | `{latencies}` | A slow Postmark or network. The next `times` matching API requests (default 1) wait `ms` (integer ≥ 1) on the clock, then go on as usual, faults included. `match` works as for a fault. Under `--clock manual` a request waits for `clock/advance`, so a test releases sends in steps. The advance answers once the requests are released, not once they are answered. A `reset` releases every held request at once. | built |
| `POST /control/faults` | `{match: {method, path, recipientDomain?}, times?, reply}` | `{faults}` | A Postmark outage or network loss. `path` is a route pattern matched like an API route. With `recipientDomain` (ASCII or punycode DNS labels with a letter, any case; no IP or trailing dot), only a request whose JSON body has a To, Cc or Bcc address at that domain matches: in the message, in any message of a batch array, or in any message of `Messages`. Body keys match without case (`docs/08` R8). A rule for one test's addresses leaves other tests' sends alone on a shared mock. On a route that sends no mail, such a rule never matches. `times` defaults to 1. `reply` is `{errorCode, status?, family?, message?}`: the envelope `apiError` builds, so only a status and code pair from `docs/02` §4.4 is accepted. `family` is one of the table families; `message` is allowed only for a summary row. 429 has no documented body and cannot be faulted yet. Or `"timeout"` (no answer until the client gives up) or `"reset"` (socket destroyed). | built |
...
| `POST /control/senders/:id/confirm` | — | the signature | The recipient clicks the confirmation link; the address may now send. 400 when already confirmed. | built (T7) |
...
| `POST /control/inbound` | Fields `{from, to?, cc?, bcc?, replyTo?, subject?, text?, html?, headers?: [{name, value}], attachments?: [{name, content (base64), contentType, contentId?}]}`, or `{mime, rcptTo}`. Both take `spamScore?` (default 0) and `spamTests?`. An address is `"a@b"` or `{email, name?}`. | `{Messages: [{MessageID, ServerID, Status}]}` | Mail arrives at an inbound address. The recipient that is a server's hash address or inbound domain picks the server; `bcc` goes only into the envelope. Rules and `InboundSpamThreshold` block it; otherwise postmock POSTs it to `InboundHookUrl`. 400 when no server owns a recipient. | built |
| `GET /control/webhooks/attempts` | `?serverId=&webhookId=&recordType=` | `{Attempts: [{ServerID, WebhookID, RecordType, Url, TraceID, Attempt, At, Headers, Body, HttpStatus \| Error, Result, NextAttemptAt}]}`. `WebhookID` is null for a server hook URL; `Result` is `success`, `retry`, `stop` or `exhausted`. | The webhook delivery log | built |
...
| `conformance` | Account token `postmock-account-token`; server ID 10 with token `postmock-server-token` and the streams `outbound`, `inbound`, `broadcast` (`seeds/conformance/00-core.ts`). Tracks add part files with fixed IDs from their range (`docs/11` §5). `70-account.ts`: server deletion enabled; domains 7000 `example.com` and 7001 `your-verified-domain.com` (the placeholder sender domain of a postmark-python example), each with a verified DKIM key; confirmed sender signature 7000 `sender@example.com`. A send `From` needs a verified domain or a confirmed signature (`docs/03` §3.4). |
...
| `conformance`, messages part | Read server 4000 `postmock read history`, token `postmock-read-server-token` (`seeds/lib/read-server.ts`): 54 outbound messages over 60 days (40 in the last 25 days, some tagged `test_tag`; 8 more 26–29 days ago), deliveries, bounces 4000–4003 (a hard bounce that suppresses `reader-4@example.com` on `outbound`, soft, transient, and a later SMTP API error to the suppressed address with a dump in the live SMTP layout), opens, clicks, and 12 inbound messages (10 processed, 1 blocked, 1 failed). Server 10 gets one processed inbound message (`seeds/conformance/40-messages.ts`). |
| `conformance`, templates part | Template server 8000 `postmock templates`, token `postmock-template-server-token` (`seeds/lib/template-server.ts`), with one standard template 8000, alias `postmock-welcome`. The core server 10 has no template (`seeds/conformance/80-templates.ts`). |
