...
| `POST /control/faults` | `{match: {method, path, recipientDomain?}, times?, reply}` | `{faults}` | A Postmark outage or network loss. `path` is a route pattern matched like an API route. With `recipientDomain` (a hostname, any case), only a request whose JSON body has a To, Cc or Bcc address at that domain matches: in the message, in any message of a batch array, or in any message of `Messages`. Body keys match without case (`docs/08` R8). A rule for one test's addresses leaves other tests' sends alone on a shared mock. `times` defaults to 1. `reply` is `{errorCode, status?, family?, message?}`: the envelope `apiError` builds, so only a status and code pair from `docs/02` §4.4 is accepted. `family` is one of the table families; `message` is allowed only for a summary row. 429 has no documented body and cannot be faulted yet. Or `"timeout"` (no answer until the client gives up) or `"reset"` (socket destroyed). | built |
