...
- postmark-cli: `SERVER_TOKEN` is the template server token (`seeds/lib/template-server.ts`). `email.template.test.ts` sends with the first listed template before any test creates one; the dotnet suite counts the templates of the core server.
...
| A fault or latency rule stays active until its `times` run out. A later test in the same process sees it. | Call `POST /control/reset` between tests. On a mock that tests share, give the rule a `recipientDomain` that only one test sends to. |
| Under `--clock manual` a sent request hangs, a bulk request never completes, and a fake bounce never arrives. | Call `POST /control/clock/advance`. Before an advance, poll `GET /control/clock` until `pending` counts the held requests. |
...
| A container on the internal network reaches the sandbox through `host.docker.internal`. Docker Desktop and OrbStack forward that to host loopback; Docker Engine on Linux maps `host-gateway` to the default bridge gateway, which a `127.0.0.1` listener does not serve. | `withContainerSandbox` binds the sandbox fronts to the bridge gateway when that address belongs to the host (Docker Engine on Linux), else to `127.0.0.1`. On the bridge gateway the fronts close every connection that does not come from the gateway container's bridge address. postmock and the trap stay on `127.0.0.1`. A daemon with another `host-gateway-ip` stops at the gateway check. |
| On a Linux CI runner the container suites stop at the gateway check: `connect ECONNREFUSED 172.17.0.1:<port>`. `os.networkInterfaces()` omits an interface that is not running. `docker0` has no carrier while no container is on the default bridge, so the bridge gateway looked foreign and the fronts bound `127.0.0.1`. The result depends on timing: java sometimes passed, because its Maven resolve container had just left the default bridge; php and cli always failed. | `gatewayTarget` (`conformance/docker.ts`) binds a test socket to the bridge gateway. A bind that succeeds proves a host address; `EADDRNOTAVAIL` means a VM engine (Docker Desktop, OrbStack). |
| Rootless Docker is not supported. Its bridge lives in the daemon's own network namespace, so a bind to the bridge gateway gives `EADDRNOTAVAIL`, the fronts bind `127.0.0.1`, and the run stops at the gateway check (`gateway cannot reach the sandbox front`). INFERRED: not run on rootless Docker. | Run the container suites on a rootful Docker Engine, Docker Desktop or OrbStack. |
