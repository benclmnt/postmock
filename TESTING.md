...
| On a Linux CI runner the container suites stop at the gateway check: `connect ECONNREFUSED 172.17.0.1:<port>`. `os.networkInterfaces()` omits an interface that is not running. `docker0` has no carrier while no container is on the default bridge, so the bridge gateway looked foreign and the fronts bound `127.0.0.1`. | `gatewayTarget` (`conformance/docker.ts`) binds a test socket to the bridge gateway. A bind that succeeds proves a host address; `EADDRNOTAVAIL` means a VM engine (Docker Desktop, OrbStack). |
