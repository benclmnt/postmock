...
- Marks follow `AGENTS.md` rule 6. The only `CAPTURED` mark here is E14 (`captures/20260916T231736Z-from-verification/`).
...
| E14 | **decided**: a batch error item is `{ErrorCode, Message}` only, as the docs show (`refs/api_email-api.md:301-313` **DOC**) and a real 400 item shows (`captures/20260916T231736Z-from-verification/02-batch-unverified-domain/response.body` **CAPTURED**). Consequence: postmark-python `SendResponse` requires `To`, `SubmittedAt` and `MessageID` (`sdk/postmark-python/postmark/models/outbound/schemas.py:96-101` **SDK**), so a python batch with one failed item still raises a validation error, against real Postmark too. Whether a 1235 item has the same shape is open (`docs/03` §8 Q14). | py vs docs |
...
| E17 | **decided**: `/deliverystats` `Bounces` items are `{Name, Count}` for `All` and `{Type, Name, Count}` per type, as the doc example shows (`refs/api_bounce-api.md:41-51` **DOC**). The python model requires `Type` and `TypeCode` on every item (`sdk/postmark-python/postmark/models/bounces/schemas.py:33-39` **SDK**). DOC outranks other SDK code (`docs/11` B3), so the python delivery stats example is a skip. | py vs docs |
...
| python | 56 examples with a placeholder ID (`0`, `42`, `12345`, `1234567`, `692560173`, or text) | No account holds the ID. postmock answers the not-found ErrorCode, or 501 where that answer is not captured. | `sdk/postmark-python/examples/sync/domains/get_domain.py:3` **SDK**; one skip file per example |
| python | 24 stats examples | `FromDate` 2024-01-01 is now more than one year ago: ErrorCode 1500. | `refs/api_overview.md:199` **DOC** |
| python | `create_domain.py`, `create_signature.py` (async, sync) | They create `example.com` and `sender@example.com`, which the conformance seed holds for every suite. | `sdk/postmark-python/examples/sync/domains/create_domain.py:5` **SDK**; `seeds/conformance/70-account.ts` |
| python | `create_inbound_rule.py`, `create_server.py`, `create_template.py`, `create_stream.py` (sync) | The examples run in file order on one postmock, async before sync. The async copy made the same placeholder object already: ErrorCode 810, 603, 1122, 1230. | `sdk/postmark-python/examples/sync/servers/create_server.py:6` **SDK** |
| python | `archive_stream.py`, `unarchive_stream.py` (async), `edit_stream.py` (sync) | Same order. The async archive runs before any create (1226). The async unarchive finds an active stream, and the sync edit finds an archived one; Postmark's answers are not captured (501, `docs/04` §2, §4). | `sdk/postmark-python/examples/async/message_streams/archive_stream.py:13` **SDK** |
